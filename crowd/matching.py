...
def pick_best_cotransporter(leg) -> Tuple[Optional[Cotransporter], str]:
    ...
    # Le client a latitude/longitude, pas l'order directement
    pickup_lat = getattr(order.customer, 'latitude', None) if order.customer else None
    pickup_lng = getattr(order.customer, 'longitude', None) if order.customer else None
