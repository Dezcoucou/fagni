...
def api_create_order(request):
    ...
    _total = _ta + _df + _sf
    _pricing = {'total_client':float(_total),'delivery_fee':float(_df),'service_fee':float(_sf),'part_pressing':float(_ta*(1-_cp)),'part_livreur':float(_df*_D('0.80')),'marge_pressing':float(_ta*(1-_cp)),'marge_livraison':float(_df*_D('0.20')),'total_fagni':float(_ta*_cp+_sf+_df*_D('0.20'))}
