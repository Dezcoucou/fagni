...
class Command(BaseCommand):
    help = "Audit payouts legs (dry-run par défaut). Options: --fix-missing, --reconcile, --fix-flags, --reverse-unpaid"
    ...
    def handle(self, *args, **opts):
        ...
        any_fix = bool(opts["fix_missing"] or opts["reconcile"] or opts["fix_flags"] or opts["reverse_unpaid"])
        ...
        for leg in legs_qs.iterator():
            ...
            has_payout = WalletTransaction.objects.filter(leg=leg, type="payout", direction="in").exists()
        ...
        def show_list(title, rows):
            ...
            for i, (order, leg, target, net, diff) in enumerate(rows[:limit], start=1):
                ...
                if verbose:
                    ...
                    self.stdout.write(f"      breakdown: payout_in={paid_in} adj_in={adj_in} adj_out={adj_out} net={net2}")
            self.stdout.write("")
        ...
        if opts["reconcile"] and mismatch:
            ...
            for order, leg, target, net, diff in mismatch:
                if getattr(order, "payment_status", None) != "paid":
                    # sécurité : on ne reconcile jamais sur unpaid
                    continue

                # recalcul après d'éventuels fix_missing
                paid_in, adj_in, adj_out, net2 = net_for_leg(leg)
                diff2 = target - net2
                if diff2 == 0:
                    continue
