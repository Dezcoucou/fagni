...
class LogisticsConfig(models.Model):
    """
    Configuration logistique globale / par ville / par réseau.

    Idée :
    - tu peux en avoir plusieurs (Abidjan, Bouaké, "Démo", etc.)
    - un seul actif à la fois (ou un par ville plus tard)
    """
    ...
    pickup_cutoff_hour = models.PositiveSmallIntegerField(
        ...
        default=10,
        help_text="Heure limite (0–23). Si collecte après cette heure, "
                  "il est difficile de garantir le 24h/48h.",
    )
    ...
    standard_sla_hours = models.PositiveIntegerField(
        ...
        default=48,
        help_text="Délai cible (en heures) pour la formule standard.",
    )
    express_sla_hours = models.PositiveIntegerField(
        "Délai express (heures)",
        default=24,
        help_text="Délai cible (en heures) pour la formule express.",
    )
...
class Order(models.Model):
    ...
    PICKUP_MODE_CHOICES = [
        ("now", "Collecte immédiate"),
        ("later", "Collecte programmée"),
    ]
    ...
    # --------- Montants de base ---------
    # Total prestations (HT ou base) – on le remplit à partir des lignes
    ...
    # Marge logistique FAGNI (livraison) – en FCFA, stocké en int
    logistic_margin = models.IntegerField(
        "Marge logistique FAGNI",
        default=0,
    )

    # --------- Montants dérivés FAGNI (modèle recommandé) ---------
    ...
    def recompute_distances_from_positions(self):
        ...
        Règle :
        - pickup   = (pickup_lat/lng ou client.lat/lng) → blanchisserie
        - delivery :
            * si delivery_lat/lng renseignés → blanchisserie → delivery
            * sinon → blanchisserie → (pickup_lat/lng ou client.lat/lng)
        """

        # 1) Point client / collecte
        origin_lat = None
        origin_lng = None
    ...
    def update_financials(self, save: bool = True):
        """
        Applique le modèle FAGNI à cette commande via compute_order_financials.
        """

        # 🔹 0) Mettre à jour la distance mission à partir des DeliveryLeg
        try:
        ...
        prestation_total = (agg["s"] or Decimal("0")).quantize(
            Decimal("0.01"),
            rounding=ROUND_HALF_UP,
        )

        # 2) Frais de livraison et coût livreur
        delivery_fee_client = (self.delivery_fee or Decimal("0")).quantize(
            Decimal("0.01"),
            rounding=ROUND_HALF_UP,
        )
        delivery_cost_driver = (self.driver_logistic_cost or Decimal("0")).quantize(
            Decimal("0.01"),
            rounding=ROUND_HALF_UP,
        )

        # 3) Service FAGNI (déjà stocké sur la commande)
        service_fee_ht = (self.service_fee or Decimal("0")).quantize(
            Decimal("0.01"),
            rounding=ROUND_HALF_UP,
        )

        # 4) Calcul complet via le moteur FAGNI
        data = compute_order_financials(
        ...
        self.fagni_revenue_ttc = data.get("fagni_revenue_ttc", Decimal("0"))

        self.amount_laundry_partner = data.get("amount_laundry_partner", Decimal("0"))
        self.amount_driver_partner = data.get("amount_driver_partner", Decimal("0"))

        self.total_client_ttc = data.get("total_client_ttc", Decimal("0"))

        if save and self.pk:
            type(self).objects.filter(pk=self.pk).update(
                prestation_total=self.prestation_total,
                total=self.total,
                delivery_fee=self.delivery_fee,
                driver_logistic_cost=self.driver_logistic_cost,
                service_fee=self.service_fee,
                commission_laundry_ht=self.commission_laundry_ht,
                commission_delivery_ht=self.commission_delivery_ht,
                logistic_margin=self.logistic_margin,
                fagni_revenue_ht=self.fagni_revenue_ht,
                vat_fagni=self.vat_fagni,
                fagni_revenue_ttc=self.fagni_revenue_ttc,
                amount_laundry_partner=self.amount_laundry_partner,
                amount_driver_partner=self.amount_driver_partner,
                total_client_ttc=self.total_client_ttc,
            )
