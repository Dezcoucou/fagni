...
def export_top_clients_csv(request):
    """
    CSV: Client,Nb Commandes,Montant Total (Top 100).
    Montant = total si présent, sinon somme(items.quantity*items.unit_price).
    """
    from django.http import HttpResponse
    ...
    # total calculé sur la relation items
    items_total = Sum(
        ExpressionWrapper(F("items__quantity") * F("items__unit_price"), output_field=DEC),
        output_field=DEC
    )
    ...
    qs = (
        ...
        .annotate(
            nb_cmd=Count("id", distinct=True),                    # éviter les doublons dus au JOIN sur items
    ...
    for row in qs:
        name = (row.get("customer__name") or "").replace('"', "").replace("\n", " ").replace("\r", " ")
